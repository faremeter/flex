...
describe("tryHold", () => {
  ...
  test("allows same authorization ID on different escrows", () => {
    const r1 = mgr.tryHold(makeParams({ escrow: ESCROW, authorizationId: 1n }), 1000n, 0n, 0n);
    const r2 = mgr.tryHold(makeParams({ escrow: ESCROW_B, authorizationId: 1n }), 1000n, 0n, 0n);
...
describe("tryHold pending count", () => {
  ...
  test("rejects when in-memory plus on-chain count hits limit", () => {
    ...
    const result = mgr.tryHold(makeParams({ authorizationId: 99n }), 100000n, 0n, 1n);
    expect(result).toEqual({
      ok: false,
      reason: "Pending settlement limit reached",
    });
...
describe("tryHold balance", () => {
  test("rejects when on-chain committed exceeds vault balance", () => {
    const result = mgr.tryHold(makeParams({ settleAmount: 1n }), 100n, 101n, 0n);
    expect(result).toEqual({
      ok: false,
  ...
  test("rejects when in-memory held leaves insufficient balance", () => {
    mgr.tryHold(makeParams({ authorizationId: 1n, settleAmount: 60n }), 100n, 0n, 0n);
    const result = mgr.tryHold(
...
describe("updateSettleAmount", () => {
  test("transitions held to settled", () => {
    mgr.tryHold(makeParams({ authorizationId: 1n, maxAmount: 200n }), 1000n, 0n, 0n);
  ...
  test("rejects if settleAmount exceeds maxAmount", () => {
    mgr.tryHold(makeParams({ authorizationId: 1n, maxAmount: 50n }), 1000n, 0n, 0n);
...
describe("sweepExpired", () => {
  test("removes held holds past their validUntilSlot", () => {
    mgr.tryHold(makeParams({ authorizationId: 1n, validUntilSlot: 100n }), 1000n, 0n, 0n);
  ...
  test("does not remove holds with null validUntilSlot", () => {
    mgr.tryHold(makeParams({ authorizationId: 1n, validUntilSlot: null }), 1000n, 0n, 0n);
  ...
  test("handles multiple expired holds", () => {
    mgr.tryHold(makeParams({ authorizationId: 1n, validUntilSlot: 100n }), 1000n, 0n, 0n);
    mgr.tryHold(makeParams({ authorizationId: 2n, validUntilSlot: 100n }), 1000n, 0n, 0n);
    mgr.tryHold(makeParams({ authorizationId: 3n, validUntilSlot: 100n }), 1000n, 0n, 0n);
    const expired = mgr.sweepExpired(100n);
...
describe("drainSubmittable", () => {
  ...
  test("sweeps expired holds first", () => {
    mgr.tryHold(makeParams({ authorizationId: 1n, validUntilSlot: 10n }), 1000n, 0n, 0n);
    mgr.tryHold(makeParams({ authorizationId: 2n }), 1000n, 0n, 0n);
    mgr.updateSettleAmount(ESCROW, 2n, 100n);
...
describe("getHeldAmount and getUnsubmittedCount", () => {
  test("getHeldAmount sums only held, settled, and submitting holds", () => {
    mgr.tryHold(makeParams({ authorizationId: 1n, settleAmount: 10n }), 1000n, 0n, 0n);
    mgr.tryHold(makeParams({ authorizationId: 2n, settleAmount: 20n }), 1000n, 0n, 0n);
    mgr.updateSettleAmount(ESCROW, 2n, 20n);
    mgr.tryHold(makeParams({ authorizationId: 3n, settleAmount: 30n }), 1000n, 0n, 0n);
    mgr.updateSettleAmount(ESCROW, 3n, 30n);
    mgr.drainSubmittable(0n);

    mgr.tryHold(makeParams({ authorizationId: 4n, settleAmount: 40n }), 1000n, 0n, 0n);
    mgr.updateSettleAmount(ESCROW, 4n, 40n);
    mgr.drainSubmittable(0n);
    mgr.markSubmitted(ESCROW, 4n, 100n);

    mgr.tryHold(makeParams({ authorizationId: 5n, settleAmount: 50n }), 1000n, 0n, 0n);
  ...
  test("getHeldAmount separates by mint", () => {
    mgr.tryHold(makeParams({ authorizationId: 1n, mint: MINT, settleAmount: 10n }), 1000n, 0n, 0n);
    mgr.tryHold(makeParams({ authorizationId: 2n, mint: MINT_B, settleAmount: 20n }), 1000n, 0n, 0n);
  ...
  test("pending count and balance are independent across mints on same escrow", () => {
    mgr.tryHold(makeParams({ authorizationId: 1n, mint: MINT, settleAmount: 80n }), 100n, 0n, 0n);
    mgr.tryHold(makeParams({ authorizationId: 2n, mint: MINT_B, settleAmount: 90n }), 100n, 0n, 0n);
