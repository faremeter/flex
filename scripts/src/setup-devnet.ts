...
import { createSolanaRpc, getAddressFromPublicKey } from "@solana/kit";
...
await configureApp();
const logger = await getLogger(["flex", "setup-devnet"]);
...
if (!OWNER_KEYPAIR_PATH) {
  logger.error("OWNER_KEYPAIR_PATH is required (the escrow owner's keypair)");
  process.exit(1);
}
...
logger.info(`Escrow:       ${escrow}`);
logger.info(`Grace period: ${GRACE_PERIOD_SLOTS} slots`);
...
logger.info(`Registering session key ${sessionKeyAddress}...`);
await sendTx(rpc, owner, [registerIx]);
logger.info(`Session key registered: PDA ${sessionKeyPDA}`);
